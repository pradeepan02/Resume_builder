...
const Template1 = ({ formData }) => {
    ...
    const renderSection = (title, content) => {
        ...
        } else if (['Certifications', 'Languages', 'Hobbies', 'Areas of Interest', 'Leadership Qualities', 'Achievements'].includes(title)) {
            return (
                <div className={`resume-section ${title.toLowerCase().replace(/\s/g, '-')}-section`}>
                    <h2>{title}</h2>
                    <ul>
                        {content.map((item, index) => (
        ...
        <div id="resume-template-1" className="resume-container">
            <div className="resume-header">
                ...
                <div className="contact-info">
                    <hr /> {/* Line above contact info */}
                    <p>
                        {formData.Email && <span><a href={`mailto:${formData.Email}`}>{formData.Email}</a></span>}
                        {formData.PhoneNumber && <span>{formData.PhoneNumber}</span>}
                        {formData.GitHub && <span><a href={formData.GitHub} target="_blank" rel="noopener noreferrer">{formData.GitHub}</a></span>}
                        {formData.LinkedIn && <span><a href={formData.LinkedIn} target="_blank" rel="noopener noreferrer">{formData.LinkedIn}</a></span>}
                    </p>
                    <hr /> {/* Line below contact info */}
                </div>
            </div>
